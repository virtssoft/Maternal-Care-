...
export const DesktopSidebar: React.FC<NavProps> = ({ activeTab, setActiveTab, onLogout, userName }) => {
  return (
    <aside className="hidden lg:flex fixed left-0 top-0 bottom-0 w-[280px] bg-white border-r border-gray-100 flex-col p-8 z-50">
      <div className="flex items-center space-x-3 mb-12">
        ...
        <span className="text-xl font-black text-gray-900 tracking-tighter uppercase">MaternalCare+</span>
      </div>

      <nav className="flex-1 space-y-2">
        <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-4 ml-4">Menu Principal</p>
        {NAV_ITEMS.map((item) => {
          const Icon = item.icon;
          const isActive = activeTab === item.id;
          return (
